...
| 23 | 2026-03-15 | arch | Per-session state management | Single SessionState object per session | Replaced 17 individual Maps keyed by sessionId with a single Map<string, SessionState>. Cleanup is centralized in cleanupSessionState(). Adding new per-session concerns requires one change (SessionState interface) instead of three (Map declaration + access + cleanup). | No |
| 24 | 2026-03-15 | arch | Release workflow skip condition | Check commit author, not message | Previous condition skipped commits starting with "chore: bump version to" which blocked releases when an agent used that message. Now checks github-actions[bot] author instead — only bot commits are skipped. | No |
| 25 | 2026-03-19 | test | Slash menu testability via buildItems export | Export buildItems() from slash-menu.ts (marked @internal) | Enables direct unit testing of menu entries without DOM setup. Same pattern as existing test-only exports. | No |
