...
export function finalizeParallelBatch(turnUsage?: { input?: number; output?: number; cacheRead?: number; cacheWrite?: number; cost?: { total?: number } } | null): void {
  ...
  // Calculate timing from actual tool data (immune to startTime resets on reopen)
  ...
  // Add stats footer showing time saved by parallelism + turn-level token usage
  ...
  if (!existingFooter) {
    ...
    // Turn-level token usage from message_end
    if (turnUsage) {
